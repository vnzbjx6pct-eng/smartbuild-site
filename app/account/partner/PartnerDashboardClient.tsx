...
import { normalizeCsvRow, type PartnerProductDraft } from "@/app/lib/partner/csv";

// Helper Interface matching the one in page.tsx
...
export default function PartnerDashboardClient({ initialProducts, isAuthorized }: PartnerDashboardClientProps) {
    // Dashboard State
    const [activeTab, setActiveTab] = useState<'overview' | 'import' | 'products'>('import');

    // Products State (Initialized from Server Props)
    const [dbProducts] = useState<PartnerProduct[]>(initialProducts);
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                setPreview([]);
                setAllData([]);
                // Trigger server revalidation refresh if needed, usually handled by Server Action revalidatePath
            } else {
                        ...
                        {activeTab === 'overview' && (
                            <div className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <StatCard title="Total Products" value={String(dbProducts.length || 0)} icon={Boxes} color="bg-blue-50 text-blue-600" />
                                            ...
                                            {dbProducts.map((p) => (
                                                <tr key={p.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${p.is_active ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
